...
@section('content')
    <div class="max-width-1200" id="main-order">
        <create-order
                ...
                @if ($order->shipping_method != \Platform\Ecommerce\Enums\ShippingMethodEnum::DEFAULT)
                    :shipping_method="'{{ $order->shipping_method }}'"
                @endif
                @if ($order->shipping_option) :shipping_option="'{{ $order->shipping_option }}'" @endif
                @if ($order->shipping_method != \Platform\Ecommerce\Enums\ShippingMethodEnum::DEFAULT && false)
